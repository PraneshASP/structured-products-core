...
const { updateContractAddresses } = require("./after-deploy");
...
async function main() {
  ...
  const structToken = await StructToken.deploy(
    CONTROLLER,
    TOKEN_NAME,
    TOKEN_SYMBOL
  );
  ...
  const structOracle = await StructOracle.deploy(PRICE_FEED);
  ...
  updateContractAddresses(fyStrategy.address);
  ...
  const sToken = await SToken.deploy("Struct SP Token", "SSP", CONTROLLER);
